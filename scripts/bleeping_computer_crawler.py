...
CONTENT_SELECTOR = ("div", {"class": "articleBody"})
URL_FILTER = "bleepingcomputer"

# 設定時間範圍（7 天內）
one_week_ago = datetime.now() - timedelta(days=2)

# 檢查資料夾是否存在，不存在則創建
output_folder = "../data/bleeping_computer"
if not os.path.exists(output_folder):
    os.makedirs(output_folder)

# 解析 RSS
feed = feedparser.parse(RSS_URL)
print(f"🔍 嘗試抓取過去 7 天內的資安新聞（來自 Bleeping Computer）\n")
        ...
        if published_time >= one_week_ago and URL_FILTER in entry.link.lower():
            ...
            try:
                ...
                if article_body:
                    ...
                    filename = f"{output_folder}/{published_time.strftime('%Y%m%d_%H%M%S')}_BleepingComputer.json"
